...
def load_and_persist_faiss(documents: list[Document], faiss_file: str, language: str):
    try:
        print(f"Memulai embedding untuk {len(documents)} dokumen...")
        embedding = CohereEmbeddings(
            model="embed-multilingual-v3.0",
            cohere_api_key="OshFeh77pulPRbckp4FMUpEPxPNtEBnUby0OaWu9"
        )
        
        # Proses dokumen per batch
        batch_size = 50
        vectordb = None
        
        for i in range(0, len(documents), batch_size):
            batch = documents[i:i + batch_size]
            print(f"Memproses batch {i//batch_size + 1}/{(len(documents)//batch_size)+1}...")
            
            if vectordb is None:
                vectordb = FAISS.from_documents(batch, embedding)
            else:
        ...
        vectordb.save_local(faiss_file)
        print(f"✅ FAISS index berhasil disimpan!")
    except Exception as e:
        print(f"❌ Error: {e}")
